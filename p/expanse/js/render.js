function render() {
  ...
  if (!["load", "start", "end"].includes(gameState)) {
    ...
    if (data.graphics > 0) {
      ...
      if (data.graphics > 2) {
        grd = ctxs.bg.createLinearGradient(0, 0, 0, cvh);
    ...
    span = {
      ...
      maxx: (Math.floor((cam.x + cvw) / tw) + 1).setBorder(0, grid.length),
      miny: (Math.floor(cam.y / tw)).setBorder(0, grid[0].length - 1),
      maxy: (Math.floor((cam.y + cvh) / tw) + 1).setBorder(0, grid[0].length),
    };
    ...
    if (data.graphics > 1) {
      ...
      if (gameState == "win") {
        ...
        ctx.drawImage(
          ...
          (player.animate - 0.5) * (cvw + (player.w / player.h) * cvh),
          cvh / 10,
          (player.w / player.h) * cvh,
          cvh,
        );
        ctx.restore();
        ctx.globalAlpha = 1;
      ...
      if (data.blocks[player?.hold?.block]) {
        ay = 0;
        if (gameState == "win") {
          ay = player.animate * (cvw - player.y);
        }
        ctx.save();
        if (player.flip < 0) {
          ctx.translate(
    ...
    if (
      ...
      if (data.graphics > 3 || data.graphics == 1) {
        ...
        for (i = 0; i < particles.length; i++) {
          ...
          ctxs.shadow.ellipse(
            ((i + Math.sin(Date.now() * sx + x) + 1) * (cvw / particles.length) + ax + cvw * 10) % cvw,
            (- (y + Date.now() * sy) + ay) % cvh + cvh,
            r,
            r,
            0, 0, 2 * Math.PI,
          );
        ...
        if (!global.debug_shadow) {
          imgd = ctxs.shadow.getImageData(0, 0, cvw, cvh);
          pix = imgd.data;
    ...
    if (gameState == "play") {
      ...
      if (global.signText) {
        ...
        if (global.lastReadSign + time0 > Date.now()) {
          ...
          ctxs.overlay.fillStyle = "#FFFFFF" + (h.length < 2 ? "0" : "") + h;
          ctxs.overlay.font = cvw * 0.08 + "px " + data.font.main;
    ...
    if (gameState == "death") {
      ...
      if (data.graphics > 0) {
        ...
        ctxs.overlay.font = (cvw * 0.001) * (player.animate * 100) + "px " + data.font.main;
      ...
      drawTranslated(
        ctxs.overlay,
        data.graphics > 0 ? ((cvw * 0.001) * (player.animate * 100)) : (cvw * 0.12),
        translateText(
          global.deathMessage,
      ...
      ctxs.overlay.font = cvw * 0.04 + "px " + data.font.main;
      ctxs.overlay.textBaseline = "top";
      ctxs.overlay.textAlign = "center";
    ...
    if (data.graphics > 1) {
      /* Draw background */
      for (i = 0; i < Math.ceil(cvw / cvh) + 1; i++) {
        ctx.drawImage(
          images.none_0,
          ((i - 1) * cvh) + ((Date.now() / 10)) % cvh,
          0,
          cvh + 1,
          cvh,
        );
    ...
    ctxs.overlay.textBaseline = "middle";
    ctxs.overlay.textAlign = "center";
    ctxs.overlay.font = cvw * 0.1 + "px " + data.font.main;
  ...
  if (global.lastGraphics + time0 > Date.now()) {
    ctxs.overlay.font = cvw * 0.03 + "px " + data.font.main;
    ctxs.overlay.textBaseline = "bottom";
    ctxs.overlay.textAlign = "left";
    h = "FF";
    if (data.graphics > 0) {
  ...
  if (global.debug_show) {
    ...
    ctxs.debug.font = cvw * 0.015 + "px Arial";
    ...
    /* Draw debug text */
    ctxs.debug.font = cvw * 0.025 + "px Arial";
    ctxs.debug.textAlign = "left";
    ctxs.debug.textBaseline = "top";
